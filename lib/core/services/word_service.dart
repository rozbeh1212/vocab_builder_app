...
/// A top-level function to parse JSON in a background isolate.
/// This function must be a static or top-level function to be used with `compute`.
List<Word> _parseWordsInBackground(String jsonString) {
  final List<dynamic> data = json.decode(jsonString);
  return data.map((word) => Word(word: word.toString())).toList();
}
...
class WordService {
  ...
  Future<List<Word>> loadWords(String category) async {
    ...
    try {
      ...
      // Decode the JSON string into a list of dynamic objects in a background isolate.
      return await compute(_parseWordsInBackground, jsonString);
    } catch (e, st) {
  ...
  Future<List<Word>> loadAllWords() async {
    ...
    for (final path in assetPaths) {
      try {
        ...
        final List<Word> words = await compute(_parseWordsInBackground, jsonString);
